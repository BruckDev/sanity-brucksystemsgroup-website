...
Recent history favors short, imperative commit subjects such as `Fix auto links and add dark mode`. This repo targets `github.com/BruckDev/sanity-brucksystemsgroup-website`; work from the `develop` branch unless told otherwise. After completing requested changes, agents are expected to create a focused commit and push it unless the user explicitly says not to push or there is a clear blocker preventing it. Do not stop at a local commit when push access is available. After each push, agents must check the GitHub/Vercel deployment status and wait for a successful result before reporting completion to the user. Do not give post-push deployment feedback early, and do not omit the preview follow-up when deployment status can be checked. Once deployment is confirmed successful, share the preview link `https://dev.brucksystems.com/`. PRs should include a brief summary, note any schema or environment variable changes, link the related issue when available, and attach screenshots for UI or Studio updates.
