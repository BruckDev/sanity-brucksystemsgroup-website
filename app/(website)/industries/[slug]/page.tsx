    ...
    <div className="space-y-16 md:space-y-20">
      ...
      <section className="grid gap-8 lg:grid-cols-[minmax(0,1.2fr)_minmax(18rem,0.8fr)]">
        ...
        <article className="border border-[color:var(--border)] bg-[color:var(--surface)] p-6 md:p-8">
          <h2 className="text-2xl font-semibold tracking-tight text-[color:var(--fg)]">Common priorities</h2>
          <div className="mt-6 flex flex-wrap gap-3">
            {(industry.priorities || []).map((priority: string) => (
              <div key={priority} className="border border-[color:var(--border)] bg-white px-4 py-2 text-sm text-[color:var(--fg)]">
                {priority}
              </div>
            ))}
          </div>
